...
def resource_path(relative_path):
    """Retorna o caminho absoluto para uso com PyInstaller ou durante desenvolvimento."""
    try:
...
class TelaComputadores(ctk.CTkToplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Computadores")
        self.attributes("-fullscreen", True)
        self.resizable(True, True)

        self.master = master
        self.computador_path = resource_path(os.path.join("data", "computadores.txt"))
        os.makedirs(resource_path("data"), exist_ok=True)
        if not os.path.exists(self.computador_path):
            open(self.computador_path, "w").close()

        self.create_widgets()
        self.load_computadores()
    ...
    def create_widgets(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de Computadores", font=("Arial", 24, "bold"))
        ...
        self.btn_add = ctk.CTkButton(self, text="Adicionar Computador", height=40,
                                     command=self.adicionar_computador)
        self.btn_add.pack(pady=20)

    def load_computadores(self):
        for widget in self.frame_lista.winfo_children():
            widget.destroy()

        with open(self.computador_path, "r") as f:
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.add_computador_widget(nome, ip)

    def add_computador_widget(self, nome, ip):
    ...
    def adicionar_computador(self):
        ...
        def salvar():
            ...
            if nome and ip:
                with open(self.computador_path, "a") as f:
                    f.write(f"{nome}|{ip}\n")
                popup.destroy()
                self.load_computadores()
            else:
    ...
    def editar_computador(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        popup.title("Editar Computador")
        popup.geometry("300x240")
        popup.grab_set()
        popup.focus_force()
        ...
        def salvar():
            ...
            if novo_nome and novo_ip:
                with open(self.computador_path, "r") as f:
                    lines = [line.strip() for line in f if line.strip()]
                with open(self.computador_path, "w") as f:
                    for line in lines:
                        if line == f"{nome_antigo}|{ip_antigo}":
        ...
        ctk.CTkButton(popup, text="Salvar Alterações", command=salvar).pack(pady=10)

    def excluir_computador(self, nome, ip):
        confirm = messagebox.askyesno("Confirmação", f"Deseja excluir o computador '{nome}'?")
        if confirm:
            with open(self.computador_path, "r") as f:
                lines = [line.strip() for line in f if line.strip()]
            with open(self.computador_path, "w") as f:
