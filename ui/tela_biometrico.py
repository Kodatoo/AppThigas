...
def resource_path(relative_path):
    """Retorna o caminho absoluto para o executável (PyInstaller) ou desenvolvimento."""
    try:
...
class TelaBiometrico(ctk.CTkToplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Biométricos")
        self.attributes("-fullscreen", True)
        self.resizable(True, True)

        self.master = master
        self.biometrico_path = resource_path(os.path.join("data", "biometricos.txt"))
        os.makedirs(resource_path("data"), exist_ok=True)
        if not os.path.exists(self.biometrico_path):
            open(self.biometrico_path, "w").close()

        self.create_widgets()
        self.load_biometricos()
    ...
    def create_widgets(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de Biométricos", font=("Arial", 24, "bold"))
        ...
        self.btn_add = ctk.CTkButton(self, text="Adicionar Biométrico", height=40,
                                     command=self.adicionar_biometrico)
        self.btn_add.pack(pady=20)

    def load_biometricos(self):
        for widget in self.frame_lista.winfo_children():
            widget.destroy()

        with open(self.biometrico_path, "r") as f:
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.add_biometrico_widget(nome, ip)

    def add_biometrico_widget(self, nome, ip):
        ...
        btn_edit = ctk.CTkButton(container, text="Editar", width=80,
                                 ...
                                 command=lambda: self.editar_biometrico(nome, ip))
        btn_edit.pack(side="right", padx=5)

        btn_delete = ctk.CTkButton(container, text="Excluir", width=80,
                                   fg_color="#FF6B6B", hover_color="#FF3B3B",
                                   command=lambda: self.excluir_biometrico(nome, ip))
        btn_delete.pack(side="right", padx=5)

    def adicionar_biometrico(self):
        popup = ctk.CTkToplevel(self)
        popup.title("Novo Biométrico")
        ...
        def salvar():
            ...
            if nome and ip:
                with open(self.biometrico_path, "a") as f:
                    f.write(f"{nome}|{ip}\n")
                popup.destroy()
                self.load_biometricos()
            else:
    ...
    def editar_biometrico(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        popup.title("Editar Biométrico")
        popup.geometry("300x240")
        popup.grab_set()
        popup.focus_force()
        ...
        def salvar():
            ...
            if novo_nome and novo_ip:
                with open(self.biometrico_path, "r") as f:
                    linhas = f.readlines()
                with open(self.biometrico_path, "w") as f:
                    for linha in linhas:
    ...
    def excluir_biometrico(self, nome, ip):
        resposta = messagebox.askyesno("Excluir", f"Excluir biométrico '{nome}'?")
        if not resposta:
            return
        with open(self.biometrico_path, "r") as f:
            linhas = f.readlines()
