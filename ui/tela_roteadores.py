...
def resource_path(relative_path):
    try:
        base_path = sys._MEIPASS  # Caminho usado pelo PyInstaller
    except Exception:
        base_path = os.path.abspath(".")  # Caminho usado no desenvolvimento
    return os.path.join(base_path, relative_path)

class TelaRoteadores(ctk.CTkToplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Roteadores")
        self.attributes("-fullscreen", True)
        self.resizable(True, True)

        self.master = master
        self.roteador_path = resource_path(os.path.join("data", "roteadores.txt"))
        os.makedirs(resource_path("data"), exist_ok=True)
        if not os.path.exists(self.roteador_path):
            open(self.roteador_path, "w").close()

        self.create_widgets()
        self.load_roteadores()
    ...
    def create_widgets(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de Roteadores", font=("Arial", 24, "bold"))
        ...
        self.btn_add = ctk.CTkButton(self, text="Adicionar Roteador", height=40,
                                     command=self.adicionar_roteador)
        self.btn_add.pack(pady=20)

    def load_roteadores(self):
        for widget in self.frame_lista.winfo_children():
            widget.destroy()

        with open(self.roteador_path, "r") as f:
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.add_roteador_widget(nome, ip)

    def add_roteador_widget(self, nome, ip):
        ...
        btn_edit = ctk.CTkButton(container, text="Editar", width=80,
                                 ...
                                 command=lambda: self.editar_roteador(nome, ip))
        btn_edit.pack(side="right", padx=5)

        btn_delete = ctk.CTkButton(container, text="Excluir", width=80,
                                   fg_color="#FF6B6B", hover_color="#FF3B3B",
                                   command=lambda: self.excluir_roteador(nome, ip))
        btn_delete.pack(side="right", padx=5)

    def adicionar_roteador(self):
        popup = ctk.CTkToplevel(self)
        popup.title("Novo Roteador")
        ...
        def salvar():
            ...
            if nome and ip:
                with open(self.roteador_path, "a") as f:
                    f.write(f"{nome}|{ip}\n")
                popup.destroy()
                self.load_roteadores()
            else:
    ...
    def editar_roteador(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        popup.title("Editar Roteador")
        popup.geometry("300x240")
        popup.grab_set()
        popup.focus_force()
        ...
        def salvar():
            ...
            if novo_nome and novo_ip:
                with open(self.roteador_path, "r") as f:
                    linhas = f.readlines()
                with open(self.roteador_path, "w") as f:
                    for linha in linhas:
    ...
    def excluir_roteador(self, nome, ip):
        resposta = messagebox.askyesno("Excluir", f"Excluir roteador '{nome}'?")
        if not resposta:
            return
        with open(self.roteador_path, "r") as f:
            linhas = f.readlines()
