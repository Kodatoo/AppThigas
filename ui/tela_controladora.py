...
def resource_path(relative_path):
    """Retorna o caminho absoluto para uso com PyInstaller ou durante o desenvolvimento."""
    try:
...
class TelaControladora(ctk.CTkToplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Controladora SEC")
        self.attributes("-fullscreen", True)
        self.resizable(True, True)

        self.master = master
        self.controladora_path = resource_path(os.path.join("data", "controladoras.txt"))
        os.makedirs(resource_path("data"), exist_ok=True)
        if not os.path.exists(self.controladora_path):
            open(self.controladora_path, "w").close()

        self.configure_layout()
        self.load_controladoras()
    ...
    def configure_layout(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de Controladoras SEC", font=("Arial", 24, "bold"))
        ...
        self.frame_lista = ctk.CTkScrollableFrame(main_frame)
        self.frame_lista.pack(expand=True, fill="both", pady=10)

        ctk.CTkButton(main_frame, text="Adicionar Controladora", command=self.adicionar_controladora, height=40).pack(pady=10)

    def load_controladoras(self):
        for widget in self.frame_lista.winfo_children():
            widget.destroy()

        with open(self.controladora_path, "r") as f:
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.add_controladora_widget(nome, ip)

    def add_controladora_widget(self, nome, ip):
        ...
        btn_edit = ctk.CTkButton(container, text="Editar", width=80,
                                 ...
                                 command=lambda: self.editar_controladora(nome, ip))
        btn_edit.pack(side="right", padx=5)

        btn_delete = ctk.CTkButton(container, text="Excluir", width=80,
                                   fg_color="#FF6B6B", hover_color="#FF3B3B",
                                   command=lambda: self.excluir_controladora(nome, ip))
        btn_delete.pack(side="right", padx=5)

    def adicionar_controladora(self):
        popup = ctk.CTkToplevel(self)
        popup.title("Nova Controladora")
        ...
        def salvar():
            ...
            if nome and ip:
                with open(self.controladora_path, "a") as f:
                    f.write(f"{nome}|{ip}\n")
                popup.destroy()
                self.load_controladoras()
            else:
    ...
    def editar_controladora(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        popup.title("Editar Controladora")
        popup.geometry("300x200")
        popup.resizable(False, False)
        popup.transient(self)
        popup.grab_set()
        ...
        def salvar_edicao():
            ...
            if novo_nome and novo_ip:
                with open(self.controladora_path, "r") as f:
                    linhas = f.readlines()
                with open(self.controladora_path, "w") as f:
                    for linha in linhas:
    ...
    def excluir_controladora(self, nome, ip):
        resposta = messagebox.askyesno("Excluir", f"Excluir controladora '{nome}'?")
        if not resposta:
            return
        with open(self.controladora_path, "r") as f:
            linhas = f.readlines()
