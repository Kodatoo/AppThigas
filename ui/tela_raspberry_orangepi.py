...
def resource_path(relative_path):
    """ Retorna o caminho absoluto para uso com PyInstaller ou desenvolvimento local """
    try:
...
class TelaRaspberryOrangePi(ctk.CTkToplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Raspberry / Orange Pi")
        self.attributes("-fullscreen", True)
        self.resizable(True, True)

        self.master = master
        self.rpi_path = resource_path(os.path.join("data", "raspberry_orangepi.txt"))
        os.makedirs(resource_path("data"), exist_ok=True)
        if not os.path.exists(self.rpi_path):
            open(self.rpi_path, "w").close()
    ...
    def create_widgets(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de Dispositivos Raspberry / Orange Pi", font=("Arial", 24, "bold"))
        ...
        self.btn_add = ctk.CTkButton(self, text="Adicionar Dispositivo", height=40,
                                     command=self.adicionar_dispositivo)
    ...
    def load_dispositivos(self):
        ...
        with open(self.rpi_path, "r") as f:
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.add_dispositivo_widget(nome, ip)

    def add_dispositivo_widget(self, nome, ip):
    ...
    def adicionar_dispositivo(self):
        ...
        def salvar():
            ...
            if nome and ip:
                with open(self.rpi_path, "a") as f:
                    f.write(f"{nome}|{ip}\n")
                popup.destroy()
                self.load_dispositivos()
            else:
    ...
    def editar_dispositivo(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        ...
        def salvar():
            ...
            if novo_nome and novo_ip:
                with open(self.rpi_path, "r") as f:
                    linhas = f.readlines()
                with open(self.rpi_path, "w") as f:
                    for linha in linhas:
    ...
    def excluir_dispositivo(self, nome, ip):
