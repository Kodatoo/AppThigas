...
class TelaAntenas(ctk.CTkToplevel):
    def __init__(self, master=None):
        ...
        self.master = master
        self.antena_path = resource_path(os.path.join("data", "antenas.txt"))
        os.makedirs(resource_path("data"), exist_ok=True)
        if not os.path.exists(self.antena_path):
            open(self.antena_path, "w").close()
    ...
    def create_widgets(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de Antenas", font=("Arial", 24, "bold"))
        ...
        self.btn_add = ctk.CTkButton(self, text="Adicionar Antena", height=40,
                                     command=self.adicionar_antena)
        self.btn_add.pack(pady=20)

    def load_antenas(self):
        for widget in self.frame_lista.winfo_children():
            widget.destroy()

        with open(self.antena_path, "r") as f:
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.add_antena_widget(nome, ip)

    def add_antena_widget(self, nome, ip):
        ...
        btn_edit = ctk.CTkButton(container, text="Editar", width=80,
                                 ...
                                 command=lambda: self.editar_antena(nome, ip))
        btn_edit.pack(side="right", padx=5)

        btn_delete = ctk.CTkButton(container, text="Excluir", width=80,
                                   fg_color="#FF6B6B", hover_color="#FF3B3B",
                                   command=lambda: self.excluir_antena(nome, ip))
        btn_delete.pack(side="right", padx=5)

    def adicionar_antena(self):
        popup = ctk.CTkToplevel(self)
        popup.title("Nova Antena")
        ...
        def salvar():
            ...
            if nome and ip:
                with open(self.antena_path, "a") as f:
                    f.write(f"{nome}|{ip}\n")
                popup.destroy()
                self.load_antenas()
            else:
    ...
    def editar_antena(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        popup.title("Editar Antena")
        popup.geometry("300x240")
        popup.grab_set()
        popup.focus_force()
        ...
        def salvar():
            ...
            if novo_nome and novo_ip:
                with open(self.antena_path, "r") as f:
                    linhas = f.readlines()
                with open(self.antena_path, "w") as f:
                    for linha in linhas:
    ...
    def excluir_antena(self, nome, ip):
        resposta = messagebox.askyesno("Excluir", f"Excluir antena '{nome}'?")
        if not resposta:
            return
        with open(self.antena_path, "r") as f:
            linhas = f.readlines()
