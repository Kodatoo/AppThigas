...
class TelaLeitorFacial(ctk.CTkToplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Leitor Facial")
        self.attributes("-fullscreen", True)
        self.resizable(True, True)

        self.master = master
        self.arquivo_path = resource_path(os.path.join("data", "leitores_facial.txt"))
        os.makedirs(resource_path("data"), exist_ok=True)
        if not os.path.exists(self.arquivo_path):
            open(self.arquivo_path, "w").close()
    ...
    def create_widgets(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de Leitores Faciais", font=("Arial", 24, "bold"))
        ...
        self.btn_add = ctk.CTkButton(self, text="Adicionar Leitor Facial", height=40,
                                     command=self.adicionar_leitor)
    ...
    def load_leitores(self):
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.add_leitor_widget(nome, ip)

    def add_leitor_widget(self, nome, ip):
        ...
        btn_edit = ctk.CTkButton(container, text="Editar", width=80,
                                 ...
                                 command=lambda: self.editar_leitor(nome, ip))
        btn_edit.pack(side="right", padx=5)

        btn_delete = ctk.CTkButton(container, text="Excluir", width=80,
                                   fg_color="#FF6B6B", hover_color="#FF3B3B",
                                   command=lambda: self.excluir_leitor(nome, ip))
        btn_delete.pack(side="right", padx=5)

    def adicionar_leitor(self):
        popup = ctk.CTkToplevel(self)
        popup.title("Novo Leitor Facial")
        ...
        def salvar():
            ...
            if nome and ip:
                with open(self.arquivo_path, "a") as f:
                    f.write(f"{nome}|{ip}\n")
                popup.destroy()
                self.load_leitores()
            else:
    ...
    def editar_leitor(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        popup.title("Editar Leitor Facial")
        popup.geometry("300x240")
        popup.grab_set()
        popup.focus_force()
        ...
        def salvar():
            ...
            if novo_nome and novo_ip:
                with open(self.arquivo_path, "r") as f:
                    linhas = f.readlines()
                with open(self.arquivo_path, "w") as f:
                    for linha in linhas:
    ...
    def excluir_leitor(self, nome, ip):
        resposta = messagebox.askyesno("Excluir", f"Excluir leitor '{nome}'?")
        if not resposta:
            return
        with open(self.arquivo_path, "r") as f:
            linhas = f.readlines()
