...
class TelaLPR(ctk.CTkToplevel):
    def __init__(self, master=None):
        ...
        self.master = master
        self.arquivo_path = resource_path(os.path.join("data", "lpr.txt"))
    ...
    def criar_layout(self):
        ...
        titulo = ctk.CTkLabel(top_frame, text="Lista de LPRs", font=("Arial", 24, "bold"))
        ...
        self.btn_add = ctk.CTkButton(self, text="Adicionar LPR", height=40, command=self.adicionar_lpr)
        self.btn_add.pack(pady=20)

    def carregar_lpr(self):
        ...
        for line in lines:
            if "|" not in line:
                continue
            nome, ip = line.split("|")
            self.adicionar_widget_lpr(nome, ip)

    def adicionar_widget_lpr(self, nome, ip):
        ...
        btn_edit = ctk.CTkButton(container, text="Editar", width=80,
                                 ...
                                 command=lambda: self.editar_lpr(nome, ip))
        btn_edit.pack(side="right", padx=5)

        btn_delete = ctk.CTkButton(container, text="Excluir", width=80,
                                   fg_color="#FF6B6B", hover_color="#FF3B3B",
                                   command=lambda: self.excluir_lpr(nome, ip))
        btn_delete.pack(side="right", padx=5)

    def adicionar_lpr(self):
        popup = ctk.CTkToplevel(self)
        popup.title("Novo LPR")
    ...
    def editar_lpr(self, nome_antigo, ip_antigo):
        popup = ctk.CTkToplevel(self)
        popup.title("Editar LPR")
        popup.geometry("300x240")
        popup.grab_set()
        popup.focus_force()
    ...
    def excluir_lpr(self, nome, ip):
        resposta = messagebox.askyesno("Excluir", f"Excluir LPR '{nome}'?")
        if not resposta:
            return
        with open(self.arquivo_path, "r") as f:
            linhas = f.readlines()
